...
export const MovieDetail = () => {
  ...
  const image = movie ? `https://image.tmdb.org/t/p/w500/${movie.poster_path}` : '';


  useEffect(() => {
    fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=a3b2ddd8968557c1f4d4c68d6987911f&language=en-US&page=1`)
    ...
    <div className="max-w-2xl text-gray-700 text-lg dark:text-white">
      ...
      <div className="my-7 flex flex-wrap gap-2">
        {movie.genres.map(genre => (
          <button
            className="cursor-text mr-2 border border-gray-200 rounded dark:border-gray-600 p-2"
            key={genre.id}
          >
            {genre.name}
          </button>
